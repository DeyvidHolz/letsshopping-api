...
export default class UserValidator extends Validator {
  ...
  public validate () : validation
  {
    ...
    if (!this.user.username.match(/[\w\.]+/g)) {
      this.validationErrors.push({ field: 'username', message: 'Invalid username.'});
    }
    ...
    if (!validator.isAlpha(this.user.firstName)) {
      this.validationErrors.push({ field: 'firstName', message: 'Invalid first name.'});
    }
  
    if (!this.user.lastName.match(/^[A-Za-z]+$/)) {
      this.validationErrors.push({ field: 'lastName', message: 'Invalid last name.'});
    }
